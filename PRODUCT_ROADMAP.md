...
3. **Reconcile "no gamification pressure" store-listing copy** with the shipped badge mechanics (narrower tension than before -- the day-streak feature this note originally also covered was removed entirely 2026-07-29). Needs a copy decision before the free listing's 5.4 step, not a code change. See "2026-07-10 session" consistency note, "2026-07-29 session" removal record.
4. **In-app help/discoverability affordance** -- one consistent idiom (tappable element vs. hint style vs. first-run hints) for the learned-star button, practice-panel controls, translation toggle, and (once re-enabled) the jump-to-example button. Needs a PM + ui-ux decision before any implementation; implementation itself is trivial-to-small once the idiom is chosen. See "2026-07-09 update". (The streak chip this item originally also covered no longer exists, 2026-07-29.)
