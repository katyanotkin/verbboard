...
- **Practice day streak** -- consecutive-day tracking, localStorage + server-mirrored (`user_practice`), 🔥 chip, wrap-up modal, JS/Python parity-tested merge logic. See "2026-07-09 update".
...
- **Streak grace/freeze (N2)** -- one free miss per streak, Plus-gated via `Settings.streak_grace_enabled`, shipped commit `1797077`. See "2026-07-27 session" (backlog text refreshed 2026-07-28 to reflect ship status).
...
2. **Label/icon fixes** from real user feedback: "Abandon" label reads too final, the "выучила"/"Learned" star button is unclear, the snail icon is unappealing. Cheap, no data-model change, still unbuilt. Predates the ver2 policy; the backlog names this (with the known-word counter) as a candidate the owner may grandfather into the free edition rather than push to Plus. See "2026-07-02 session" #5, "2026-07-15 addendum".
3. **Reconcile "no gamification pressure" store-listing copy** with the shipped streak/badge mechanics (and the further gamification already planned). Needs a copy decision before the free listing's 5.4 step, not a code change. See "2026-07-10 session" consistency note.
4. **In-app help/discoverability affordance** -- one consistent idiom (tappable element vs. hint style vs. first-run hints) for the streak chip, learned-star button, practice-panel controls, translation toggle, and (once re-enabled) the jump-to-example button. Needs a PM + ui-ux decision before any implementation; implementation itself is trivial-to-small once the idiom is chosen. See "2026-07-09 update".
5. **Nightly auto-generation of candidates from top demand signals (N8).** Admin efficiency, not user-facing -- turns the review pipeline from pull to push. Needs a Cloud Scheduler job and AI-spend guardrails; human promote/review step stays. See "2026-07-10 session" N8.
6. **"Redesign every screen"** -- one user's open-ended instinct that the whole UI needs a professional pass. Not a scoped item; parked pending a dedicated UX audit, not something to size yet. See "2026-07-02 session" #5.
