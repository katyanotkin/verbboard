"""
Backfill verb infinitive/lemma translations for all verbs in Firestore.

Translates each verb's lemma into all supported UI languages except the verb's
...
Usage (run from project root, needs GCP auth + ANTHROPIC_API_KEY in .env):
    ...
    python -m tools.backfill_lemma_translations --language ru
    python -m tools.backfill_lemma_translations --language all --dry-run
    python -m tools.backfill_lemma_translations --language en --target-lang ru
"""
...
from core.translation_service import SUPPORTED_LANGUAGES, translate_lemma
...
def process_verb(
    ...
    logger.info("  translating lemma %r -> %s", lemma, effective_targets)
    if dry_run:
        return True
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backfill verb infinitive/lemma translations.")
    parser.add_argument(
        "--language",
        default="all",
