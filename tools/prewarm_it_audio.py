"""One-off script: pre-generate TTS audio (both voices) for every Italian
verb currently in Firestore, using the same _warm_verb_audio() pipeline the
admin candidate flow uses on verb add/regenerate.

The seeding script (tools/seed_it_verbs.py) deliberately skipped this,
relying on on-demand generation as a fallback -- this closes that gap so
audio is ready immediately instead of generating lazily on first play.

Usage:
    .venv/bin/python -m tools.prewarm_it_audio
"""
...
async def main() -> None:
    ...
    print(f"Pre-warming audio for {len(verbs)} Italian verbs...")
