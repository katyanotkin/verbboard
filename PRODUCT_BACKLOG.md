...
The superficial-match finding above (client-side substring search, no real grammatical link) got a live-data audit once Italian shipped -- 20 real verbs with a compound `passato_prossimo` tense (e.g. "ho parlato", two tokens, unlike every other language's single-token forms), a real stress test for the matching logic. Full findings from that audit belong here once the investigating pass reports back; not filled in yet as of this entry.
