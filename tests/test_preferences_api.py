...
- GET /api/preferences returns all fields (ui_language, learning_language,
  practice_session_size, practice_min_plays) -- even when not set (returns nulls)
...
- POST /api/preferences rejects invalid practice_min_plays value
- POST /api/preferences accepts partial payloads (only one field)
- POST /api/preferences accepts all valid practice_min_plays values
...
def test_get_preferences_returns_all_fields() -> None:
    """GET must return all preference fields even if unset."""
    ...
    assert "practice_min_plays" in payload
...
def test_post_preferences_rejects_invalid_practice_min_plays() -> None:
    """practice_min_plays values outside {1, 3, 5, 8, 'all'} must be rejected with 422."""
    response = client.post(
        "/api/preferences",
        headers=AUTH,
        json={"practice_min_plays": 7},
    )
    assert response.status_code == 422
...
@pytest.mark.parametrize("value", [1, 3, 5, 8, "all"])
def test_post_preferences_accepts_valid_practice_min_plays(value: int | str) -> None:
    """All valid practice_min_plays values must be accepted."""
    response = client.post(
        "/api/preferences",
        headers=AUTH,
        json={"practice_min_plays": value},
    )
    assert response.status_code == 200
