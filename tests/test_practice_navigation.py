...
def test_home_browse_practice_btn_language_matches_selection(client: TestClient) -> None:
...
# verbs page: practice-nav-chip

def test_verbs_practice_nav_chip_present_when_enabled(client: TestClient, monkeypatch) -> None:
    monkeypatch.setattr("app.routes.verbs.PRACTICE_LOOP_ENABLED", True)
    monkeypatch.setattr("app.routes.verbs.load_entries_for_language", lambda **kw: _stub_entries())
    html = client.get("/verbs?language=en").text
    assert "practice-nav-chip" in html


def test_verbs_practice_nav_chip_links_to_practice_panel(client: TestClient, monkeypatch) -> None:
    monkeypatch.setattr("app.routes.verbs.PRACTICE_LOOP_ENABLED", True)
    monkeypatch.setattr("app.routes.verbs.load_entries_for_language", lambda **kw: _stub_entries())
    html = client.get("/verbs?language=en").text
    assert 'href="#practice-panel"' in html


def test_verbs_practice_nav_chip_absent_when_disabled(client: TestClient, monkeypatch) -> None:
    monkeypatch.setattr("app.routes.verbs.PRACTICE_LOOP_ENABLED", False)
    monkeypatch.setattr("app.routes.verbs.load_entries_for_language", lambda **kw: _stub_entries())
    html = client.get("/verbs?language=en").text
    assert "practice-nav-chip" not in html


# verbs page: search submit button not disabled

def test_verbs_search_submit_not_disabled_on_load(client: TestClient, monkeypatch) -> None:
    monkeypatch.setattr("app.routes.verbs.load_entries_for_language", lambda **kw: _stub_entries())
    html = client.get("/verbs?language=en").text
...
def test_verbs_search_submit_btn_has_no_disabled_attr(client: TestClient, monkeypatch) -> None:
    monkeypatch.setattr("app.routes.verbs.load_entries_for_language", lambda **kw: _stub_entries())
    html = client.get("/verbs?language=en").text
...
def test_verbs_search_uses_home_search_placeholder_en(client: TestClient, monkeypatch) -> None:
    monkeypatch.setattr("app.routes.verbs.load_entries_for_language", lambda **kw: _stub_entries())
...
def test_verbs_search_placeholder_per_ui_lang(client: TestClient, monkeypatch, ui_lang: str) -> None:
    monkeypatch.setattr("app.routes.verbs.load_entries_for_language", lambda **kw: _stub_entries())
...
def test_verbs_search_placeholder_key_absent_from_json_file(lang: str) -> None:
    ...
    assert "verbs.search_placeholder" not in raw, (
